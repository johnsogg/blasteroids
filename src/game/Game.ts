...
export class Game {
  ...
  private update(deltaTime: number): void {
    ...
    this.gameObjects.forEach((obj) => {
      if (obj.type === "ship") {
        ...
        obj.age = (obj.age || 0) + deltaTime;
        obj.warpAnimationProgress = Math.min(1, (obj.age || 0) / 1.0); // 1 second animation
    ...
    this.gameObjects = this.gameObjects.filter((obj) => {
      if (obj.type === "bullet") {
        ...
        if ((obj.age || 0) >= 3.0) {
          // Spawn the gift when warp bubble finishes opening
          this.spawnGiftFromWarp(obj);
          return false;
        }
        return true;
      } else if (obj.type === "warpBubbleOut") {
        // Remove when closing animation is complete (1 second)
        return (obj.age || 0) < 1.0;
      } else if (obj.type === "gift") {
        // Check if gift has expired (5 seconds after spawn)
        if ((obj.age || 0) >= 5.0) {
          // Create closing warp bubble along gift's path
          this.createClosingWarpBubble(obj);
          return false;
        }
        return true;
      }
  ...
  private checkCollisions(): void {
    ...
    for (const gift of gifts) {
      for (const warpOut of warpBubblesOut) {
        if (Collision.checkCircleCollision(gift, warpOut)) {
      ...
      for (let i = 0; i < fragmentCount; i++) {
        ...
        this.gameObjects.push({
          position: asteroid.position.add(
            new Vector2((Math.random() - 0.5) * 40, (Math.random() - 0.5) * 40),
          ),
  ...
  private spawnGiftFromWarp(warpBubble: GameObject): void {
    // Calculate a trajectory toward a random point (not necessarily the ship)
    const targetX = Math.random() * this.canvas.width;
    const targetY = Math.random() * this.canvas.height;

    const direction = new Vector2(
      targetX - warpBubble.position.x,
      targetY - warpBubble.position.y,
    ).normalize();
    ...
    this.gameObjects.push({
      ...
      giftCollectionDeadline: performance.now() + 5000, // 5 second collection window
    });
  }

  private createClosingWarpBubble(gift: GameObject): void {
    // Calculate position ahead of the gift along its trajectory
    const futurePosition = gift.position.add(gift.velocity.multiply(0.5)); // 0.5 seconds ahead
