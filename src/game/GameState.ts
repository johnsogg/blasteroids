...
export class GameState {
    ...
    private updateUI(): void {
        ...
        if (highScoreElement) {
            const formattedHighScore = this.formatRetroNumber(this._highScore.toString());
            highScoreElement.innerHTML = formattedHighScore;
        }
    }

    private updateLivesDisplay(): void {
        const livesCanvas = document.getElementById('livesCanvas') as HTMLCanvasElement;
        ...
        const maxLives = 3;
        const shipSize = 10; // 50% of normal ship size (20px -> 10px)
        const spacing = 30;
